...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        '''
        def count (text):
            text_words = getWords(text)
            acc = 0
            for word in text_words:
                if word in self.valid_words:
                    acc += 1
            return acc
        clear = (0,self.message_text)
        clear_count = count(self.message_text)
        for s in range(1,26):
            candidate = self.apply_shift(s)
            candidate_count = count(candidate)
            if candidate_count > clear_count:
                clear = (s,candidate)
                clear_count = candidate_count 
        return clear
