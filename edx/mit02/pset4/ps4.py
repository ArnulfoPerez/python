...
def generate_models(x, y, degs):
    ...
    """
    # TODO
    return [np.polyfit(np.array(x), np.array(y), degree) for degree in degs] 
...
def r_squared(y, estimated):
    ...
    # TODO
    data = np.array(y)
    model = np.array(estimated)
    mean = np.mean(data)
    error= data - model
    var = data - mean
    return 1 - np.dot(error,error)/np.dot(var,var)
...
def evaluate_models_on_training(x, y, models):
    ...
    # TODO
    for model in models:
        estimated = np.polynomial.polynomial.polyval(x,model[::-1])
